...
namespace VoiceAssistant
{
    ...
    {
        #region Declarations
        private string _serviceUrl = "https://api.au-syd.speech-to-text.watson.cloud.ibm.com/instances/41b95890-d7ba-47a4-9bba-d8f4a271c642";
        private string _iamApikey = "GxeraCZL6wVoX8Dc7oRCWXzrCcD612X7UBDcdNHcBkjU";
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        welcomeScript.UserSpeech.text = text;
                        //if speech processing completed
                        if (res.final == true)
                        {             
                            //"Processing";
                            welcomeScript.CurrentState = State.AnalyzingTone;
                            toneAnalyzer.StartAnalyzer(welcomeScript.UserSpeech.text);
                            Active = false;

                            //save user array 
                            string utteranceText = text.Substring(0, text.Length - 15);
                            ...
                            {
                                Debug.Log(utterance.Text + "  " +utterance.User);
                            }

                            //check for good bye 
                            if (text.Contains("goodbye") || text.Contains("good bye"))
                            {
                                //toneAnalyzer.StartAnalyzerConversation(utterances);
                                Debug.Log("\n\n Exiting \n\n");
